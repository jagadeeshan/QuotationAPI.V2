...
public class InvoiceCalcController : ControllerBase
{
    private readonly QuotationDbContext _db;
    public InvoiceCalcController(QuotationDbContext db) => _db = db;

    [HttpGet]
    public async Task<IActionResult> GetAll()
    {
        var records = await _db.InvoiceCalcRecords
    ...
    {
        var record = await _db.InvoiceCalcRecords.FindAsync(id);
        if (record != null)
        {
    ...
    {
        var record = new InvoiceCalcRecord
        {
    ...
    {
        var source = await _db.InvoiceCalcRecords.FindAsync(id);
        if (source == null) return NotFound();
        var clone = new InvoiceCalcRecord
        {
