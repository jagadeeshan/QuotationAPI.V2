BEGIN TRY
  ...
  DECLARE @unitId INT;
  SELECT @unitId = Id FROM LovItems WHERE Parentvalue IS NULL AND Name = N'Unit';
  IF @unitId IS NULL
  BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM LovItems WHERE Parentvalue = @unitId AND Value = 1)
    INSERT INTO LovItems (Parentname, Parentvalue, Name, Value, Description, Itemtype, Displayorder, Isactive, Createdby, Updatedby, Createddt, Updateddt)
    VALUES (N'Unit', @unitId, N'Inches', 1, N'Inches', N'CATEGORY_VALUE', 1, N'Y', N'system', N'system', @now, @now);

  IF NOT EXISTS (SELECT 1 FROM LovItems WHERE Parentvalue = @unitId AND Value = 2)
    INSERT INTO LovItems (Parentname, Parentvalue, Name, Value, Description, Itemtype, Displayorder, Isactive, Createdby, Updatedby, Createddt, Updateddt)
    VALUES (N'Unit', @unitId, N'Milli Meter', 2, N'Millimeter', N'CATEGORY_VALUE', 2, N'Y', N'system', N'system', @now, @now);

  IF NOT EXISTS (SELECT 1 FROM LovItems WHERE Parentvalue = @unitId AND Value = 3)
    INSERT INTO LovItems (Parentname, Parentvalue, Name, Value, Description, Itemtype, Displayorder, Isactive, Createdby, Updatedby, Createddt, Updateddt)
    VALUES (N'Unit', @unitId, N'Centi Meter', 3, N'Centimeter', N'CATEGORY_VALUE', 3, N'Y', N'system', N'system', @now, @now);

  DECLARE @jointId INT;
  SELECT @jointId = Id FROM LovItems WHERE Parentvalue IS NULL AND Name = N'Joint';
  IF @jointId IS NULL
  BEGIN
    INSERT INTO LovItems (Parentname, Parentvalue, Name, Value, Description, Itemtype, Displayorder, Isactive, Createdby, Updatedby, Createddt, Updateddt)
  ...
  WHILE @i <= 15
  BEGIN
    IF NOT EXISTS (SELECT 1 FROM LovItems WHERE Parentvalue = @modelId AND Value = @i)
      INSERT INTO LovItems (Parentname, Parentvalue, Name, Value, Description, Itemtype, Displayorder, Isactive, Createdby, Updatedby, Createddt, Updateddt)
      VALUES (
        N'Model',
        @modelId,
...
FROM LovItems c
JOIN LovItems v ON v.Parentvalue = c.Id
